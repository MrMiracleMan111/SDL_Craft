...
float time_of_day()
{
    ...
    float t;
    t = (double)(SDL_GetTicks()*100.0) / 1000.0;
    t = t / g->day_length;
    t = t - (int)t;
    return t;
}
...
void handle_movement(double dt, SDL_Event *event)
{
    ...
    if (!g->typing)
    {
        ...
        if (GetKey(g->window, CRAFT_KEY_FORWARD, keyboard_state) || GL_TRUE)
            sz--;
        if (GetKey(g->window, CRAFT_KEY_BACKWARD, keyboard_state))
            sz++;
        if (GetKey(g->window, CRAFT_KEY_LEFT, keyboard_state))
            sx--;
        if (GetKey(g->window, CRAFT_KEY_RIGHT, keyboard_state))
            sx++;
        if (GetKey(g->window, SDL_SCANCODE_LEFT, keyboard_state))
            s->rx -= m;
        if (GetKey(g->window, SDL_SCANCODE_RIGHT, keyboard_state))
            s->rx += m;
        if (GetKey(g->window, SDL_SCANCODE_UP, keyboard_state))
            s->ry += m;
        if (GetKey(g->window, SDL_SCANCODE_DOWN, keyboard_state))
            s->ry -= m;
    }
    float vx, vy, vz;
    ...
    if (!g->typing)
    {
        if (GetKey(g->window, CRAFT_KEY_JUMP, keyboard_state) || GL_TRUE)
        {
            if (g->flying)
            {
                vy = 1;
...
int main(int argc, char **argv)
{
        ...
        SDL_setenv("SDL_EVDEV_DEVICES", "0:/dev/input/event2", 0);
    #endif
